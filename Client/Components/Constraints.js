...
export default class Constraints extends React.Component {
  ...
  submitConstraints = async () => {
    ...
    try {
      ...
      console.log(err);
      this.setAlertsState('responseError', true, resJson.error.msg);
      //alert(err);
    }
  };
  ...
  createAllAlerts() {
    ...
    this.addAlertToarray(alerts, 'constraintsNotApproved', 'Error');
  ...
  async getConstraints(token) {
    ...
    try {
      ...
      if (json.success) {
        this.setState({
          data: json.weeklyConstraints
            ? json.weeklyConstraints
            : this.state.data,
          specificConstraints: json.specificConstraints,
        });
