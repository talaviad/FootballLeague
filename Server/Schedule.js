module.exports = class Schedule { // tal's old state
    ...
    computeScheduling = async () => {
        ...
        for(let teamId in teamsConstraints) {
          ...
          for(let i=0; i<pitchConstraints.length; i++) {
            ...
            console.log('numOfDays of ' + teamId + ': ' + teamsConstraints[teamId].constraints[i].length);
            for(let j=1; j<pitchConstraints[0].length; j++) {
                console.log('teamsConstraints[teamId].constraints[i][j-1]: ' + teamsConstraints[teamId].constraints[i][j-1]);
                console.log('pitchConstraints[i][j]: ' + pitchConstraints[i][j]);
                console.log('(teamsConstraints['+teamId+'].constraints['+i+']['+(j-1)+'] && pitchConstraints['+i+']['+j+']: ' + (teamsConstraints[teamId].constraints[i][j-1] && pitchConstraints[i][j]));
                teamsConstraints[teamId].constraints[i][j-1] = (teamsConstraints[teamId].constraints[i][j-1] && pitchConstraints[i][j]);
            }
          }
        }
        ...
        //     }
        // }
        console.log('teamsConstraints[1].constraints: ' + teamsConstraints[1].constraints);
        ...
        let teamsNumbers = [];
        let gamesIdsToUsers = {};
        for (let i=1; i<=arr[0].length*2; i++) {   //should also add a consider to pitch constraints
            for (let j=i+1; j<=arr[0].length*2; j++) {
                ...
                for (let k=0; k<constraintsA.length; k++) {
                    for (let m=0; m<constraintsA[0].length; m++) {
                        let canPlayInThatTime = (constraintsA[k][m] === 1 && constraintsB[k][m] === 1)? true : false;
                        matches[teamVsTeamId][(k*constraintsA[0].length) + m] = canPlayInThatTime;
                        ...
                        if (canPlayInThatTime) {
                            ...
                            console.log('teamsConstraints[i].teamName: ' + teamsConstraints[i].teamName);
                            gamesIdsToUsers[teamVsTeamId] = [{ user: teamsConstraints[i].teamCaptainUser, teamName: teamsConstraints[i].teamName }, { user: teamsConstraints[j].teamCaptainUser, teamName: teamsConstraints[j].teamName }];
                        }
                    }
                }
                if (!matchFound) {
                    console.log('in pre-checking we found false');
                    console.log('Problem - teamA: ' + i + ', teamB: ' + j + ', cannot play against each other');
                    return;
                }
                teamVsTeamId++;
            }
        ...
        let playedGames = {}
        let games = [];
        let ans = this.checkSchedule(arr, 0, matrix, arr[0].length*2, games, matches, teamsNumbers, playedGames);
        ...
        if (ans.success) {
          ...
          return {
            success: false,
            error: {
              msg: 'server could not find any schedule with the current constraints'
            }
          }
        }
      }

      checkSchedule(arr, rowIndex, matrix, originalLength, games, matches, teamsNumbers, playedGames) {
        ...
        if (originalLength === 4 || (arr[0].length-rowIndex)*2 === 4) {
            ...
            for (let i=0; i<3; i++) {
                ans = this.checkCurrSchedule(arr, matrix, games, matches, teamsNumbers, playedGames);
                if (ans.success) {
        ...
        for (let i=0; i<(currLength-1); i++) {
            ...
            ans = this.checkSchedule(arr, rowIndex+1, matrix, originalLength, games, matches, teamsNumbers, playedGames);
            if (ans.success) {
                //this.setWinSchedule(games, arr);
                this.rotateLeft(arr, i, rowIndex);
                ans.teamsNumbers = teamsNumbers;
    ...
    checkCurrSchedule(arr, matrix, games, matches, teamsNumbers, playedGames) {
        ...
        for (let i=0; i<length-1; i++) {
            ...
            if (!this.checkConstraints(arr, 0, matrix, matches, schedule[i], playedGames)) {
                //this.setWinSchedule(games, arr);
